...
from .legacy import patch_driver  # noqa: F401
from .legacy.immovlan_url_scraper import ImmovlanUrlScraper

DEFAULT_TEMPLATE = os.getenv(
    "IMMOVLAN_HOUSES_BASE_URL_TEMPLATE",
    "https://www.immovlan.be/en/real-estate/for-sale/house?search={town}"
)
...
def run(max_pages: int = None, headless: bool = None, delay_min: float = 1.0, delay_max: float = 2.0):
    ...
    logger.info("🏠 Houses scrape | towns=%d | headless=%s | max_pages=%s", len(df), headless, max_pages)
    out_root = os.path.join(cfg.raw_dir, datetime.now().strftime("%Y/%m/%d"), "houses")
    ...
    logger.info("✅ Houses URL scraping done.")
    return "houses_ok"
